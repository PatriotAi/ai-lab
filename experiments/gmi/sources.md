...
- **Обмеження (початковий harvest, крок 2):** тоді **WebFetch був блокований (HTTP 403)** — прямий
  fetch первинних PDF недоступний, тож числа спершу **search-derived**. **Станом на 2026-07-18**
  мережа переважно розблокувалась (403 зник для більшості доменів; лишилось ~3 egress-блоковані).
  Обхід (`source-research-harvest` info-max): крос-валідація незалежними **WebSearch** (у субагенті).
  **T6 закрито 2026-07-18**
  (`docs/reviews/2026-07-18-diagnostics-audit.md` §3: 10/10 звірено — **7 CONFIRMED · 3 UNREACHABLE**
  (egress-блок доменів; вторинні узгоджені) **· 0 розбіжностей**). Незалежна ре-звірка ключових
  чисел (субагент, WebSearch) — узгоджена з тим звітом → **позначки звірки знято** з тексту нижче.
...
- **[C] ✅ Мозок ≈1.26× межі Ландауера (для обчислювальної частки):** бюджет мозку ~15–20 Вт
  ...
  саме ~3 Вт-частка ≈ **1.26× теоретичного мінімуму** (операційна ефективність ~79%). Тобто мозок
  майже вичерпав термодинамічний резерв для своєї обчислювальної роботи. **T6: ✅ CONFIRMED**
  (звіт 2026-07-18 §3, тв.1; ре-звірка субагентом узгоджена — 79% ≡ 1.26×). *Примітка строгості:
  це **модельна оцінка препринту 2025** (arXiv, ще не рецензовано в журналі), а не фізична константа.*
  [Neuromorphic architecture of brain (arXiv 2508.03191)](https://arxiv.org/abs/2508.03191) ·
  [NeuroscienceNews — 79% ефективність + розклад потужності](https://neurosciencenews.com/information-processing-brain-efficiency-31090/)
...
| 3. Gap-map ефективності | ✅ | числа **звірені** (T6, 2026-07-18, звіт diagnostics-audit §3: 7 CONFIRMED/3 UNREACHABLE); множник — чесно як діапазон [C] |
