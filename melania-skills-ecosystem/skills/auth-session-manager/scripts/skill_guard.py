#!/usr/bin/env python3
"""skill_guard.py — захист auth-session-manager від регресій.
...
def snapshot():
    ...
    snap = {
        ...
        "gate_rows": gate_rows(text),
    }
    SNAP_PATH.parent.mkdir(exist_ok=True)
    SNAP_PATH.write_text(json.dumps(snap, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"✅ Snapshot: {snap['total_lines']} рядків · MD5: {snap['md5'][:16]} · security-gate рядків: {snap['gate_rows']}")
...
def validate():
    ...
    if "AES-GCM" not in text:
        errors.append("AES-GCM шифрування видалено — критичне порушення безпеки!")
    if "НІКОЛИ" not in text:
        errors.append("Core Rule безпеки ослаблено!")
    if errors:
        print(f"❌ ЗАБЛОКОВАНО — {len(errors)} помилок:")
        for e in errors: print(f"  ✗ {e}")
        sys.exit(1)
    print(f"✅ VALID — {cur} рядків · усі терміни присутні · security-gate рядків: {gate_rows(text)}")
