...
## Critical Facts
- **[C] RAG-пошук ламається при розбіжності embeddings-провайдера.** Вузол Vector Store Retrieval має використовувати той самий embeddings-вузол (напр. `embeddingsOpenAi` чи `embeddingsAnthropic`), що й Vector Store Insert — інакше вектори з різних просторів не зіставляються і пошук повертає нерелевантні результати.
- **[C] n8n має нативні AI Agent вузли (v1.x+).** Вони підтримують multi-step reasoning, памʼять між повідомленнями (Memory Buffer Window) і динамічний виклик інструментів — це заміна ручному HTTP Request до LLM, а не лише зручність.
- **[C] Вузол Structured Output (`outputParserStructured`) дає надійний JSON з LLM.** Конфігурація `schemaType: fromJson` усуває потребу в ручному парсингу тексту моделі регулярними виразами.
- **[C] Кожен `id` ноди у workflow JSON мусить бути унікальним.** Дублікат `id` або пропущений `typeVersion` — часта причина того, що n8n не може коректно імпортувати згенерований workflow.
...
- **v2.10.0** (2026-07-26) — Секція **Critical Facts**: фактичні твердження скіла винесено окремо й протеговано [C] за Core Rule 14 (claim-evidence). Лише додавання.
