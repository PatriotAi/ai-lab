...
# Melania — Skill Master Administrator — v2.23.0
> **v2.23.0** · Master Administrator: Меланія · `references/CHANGELOG.md`
...
## Critical Facts
- **[C] Guard — SELF-BOUND.** Запуск чужої копії `skill_guard.py` з аргументом-іменем валідує ЧУЖИЙ скіл, а не названий; аргумент ігнорується. Здобуто болем, не читанням коду.
- **[C] Завантажувач скіл-стору відхиляє крапкові шляхи.** `.snapshots` у теці скіла блокує встановлення пакета — тому пакування йде лише через `maintain.py package`.
- **[C] Монтування скілів буває застарілим у середині сесії.** Нові скіли з'являються, а оновлення наявних — ні; версія на диску не є доказом найновішої.
- **[E] В ai-lab гейт цілісності — `maintain.py verify`:** MANIFEST, guard усіх скілів, safety-скан, актуальність лічильників і доказовість тверджень; порушення дає ненульовий код виходу. (`melania-skills-ecosystem/scripts/maintain.py`, `tests/run-tests.sh`, 2026-07-26)
...
14. Claim-evidence  фактичне твердження (секція «Critical Facts» у скілах, фактичні заяви в докам) несе тег [E] перевірено ділом / [C] обґрунтоване, машинно не перевірене / [S] гіпотеза; [E] — ЛИШЕ з вказівником на реальну перевірку, і вказівник мусить ІСНУВАТИ на диску: формат вказівника ще не доказ (саме так пройшли «claimed-but-missing» evals), доказ має бути знаходжуваним. Секція «Critical Facts» ОБОВ'ЯЗКОВА в кожному скілі — скіл без неї не має що пред'явити гейту, і «немає тверджень» перестає бути способом обійти правило. Тест до [E] береться на НАЙВАЖЧОМУ випадку в межах твердження і має падати, якщо твердження хибне (falsification-first). Нема доказу → звузь формулювання до фактично зробленого або постав [C]/[S]; широку обіцянку не лишати. Директиви НЕ тегуються — правило не буває істинним чи хибним. Самоперевірка цей клас НЕ ловить (5 випадків, жоден): доказ дає незалежний перевіряч — реальний прогін, машинний гейт (maintain.py verify) або ізольований агент
15. Self-verifying  крок протоколу, який ЗВІТУЄТЬСЯ словами, мусить мати машинну перевірку — інакше він порушується, а звіт цього не показує (чотири дрейфи пережили хвилю попри чесні звіти «H1-синхрон виконано»). Звіт не є доказом виконання. Покриття перевірки визначає ПЕРЕЛІК ІНЦИДЕНТІВ, які гейт обіцяє закрити, а не число: «canary ×3» — це число, і саме тому три чесні канарки пройшли, а дірок було чотири. Кожну нову перевірку прогнати на ЗЛАМАНОМУ стані (має спіймати) і на чистому корпусі (має мовчати) — перевірка, що лише проходить на чистому, нічого не доводить
```
...
## Зміни
- **v2.23.0** (2026-07-26) — **Core Rule 15 Self-verifying**: крок протоколу, який звітується словами, мусить мати машинну перевірку — інакше він порушується, а звіт цього не показує. Підстава емпірична: чотири дрейфи пережили попередню хвилю ПОПРИ чесні звіти про виконання (H1-синхрон заявлено — не зроблено; крос-посилання П.6/П.7/П.8 з'їхали; «8 дисциплін» при 10; виправлення канону про hooks не дійшло до копії). Друга половина правила: покриття перевірки визначає ПЕРЕЛІК ІНЦИДЕНТІВ, а не число — «canary ×3» дало три чесні канарки при чотирьох дірках. Операціоналізовано в `maintain.py verify` чотирма перевірками (версійна тріада · крос-посилання за ідентифікатором · лічильники про інший файл · текстова гігієна), кожну фальсифікаційно прогнано на зламаному стані. Виправлено успадкований омоглиф у слові `aдаптовано` (латинська `a` замість кириличної) у записі v2.6.0 — знайдено новою перевіркою. Лише додавання й корекція.
- **v2.22.0** (2026-07-26) — **Core Rule 14 посилено до механічно перевірюваного**: (1) вказівник `[E]` мусить ІСНУВАТИ на диску — формат вказівника не є доказом, і саме тому випадок «evals заявлені з v1.2.1, артефакту немає» проходив гейт без заперечень; (2) секція «Critical Facts» тепер ОБОВ'ЯЗКОВА в кожному скілі — доти гейт покривав 2 скіли з 28, решта проходили порожньо («немає тверджень» було способом обійти правило). Додано власну секцію Critical Facts. Виправлено H1: лишався `v2.20.0` при банері `v2.21.0` — заявлений у попередній хвилі H1-синхрон не відбувся. Лише додавання й корекція.
...
- **v2.6.0** (2026-06-02) — адаптовано з zip: синхронізація з аудитом.
