...
## Critical Facts
- **[E] Гейт доказовості ловить три підробки:** факт без тега, `[E]` без вказівника і `[E]` зі вказівником на неіснуючий файл; коректний текст і директива в `Core Rule` проходять чисто. (`melania-skills-ecosystem/scripts/maintain.py`, `tests/run-tests.sh`, 2026-07-26)
- **[C] Самоперевірка цей клас дефекту не ловить.** У п'яти задокументованих випадках усі до одного спіймали перевіряч ПОЗА автором: реальний прогін, зовнішній рецензент, ізольований агент, форензик-аудит.
- **[C] Тест, написаний під формулювання, підтверджує формулювання, а не істину.** Тест на омоглифи брав повноширинну форму, яку NFKC зводить, — і «проходив», доки кирилична літера лишалась незловленою.
- **[C] Гейт без canary — сам неперевірене твердження.** Доки не зламаєш навмисно, «гейт працює» лишається заявою, а не фактом.
...
| **Доказ має ІСНУВАТИ** | Формат вказівника — ще не доказ. Випадок «evals заявлені з v1.2.1, артефакту не існувало» пройшов би будь-яку формат-перевірку; тому `[E]` мусить називати шлях, який реально є на диску — доказ має бути **знаходжуваним** |
...
- **v1.8.0** (2026-07-26) — Секція **Critical Facts** (Core Rule 14) + посилення шару Claim-Evidence: **доказ має ІСНУВАТИ**, а не лише мати правильний формат — вказівник `[E]` на неіснуючий файл проходив формат-перевірку, тобто випадок «evals заявлені, артефакту немає» гейт пропускав. Лише додавання.
